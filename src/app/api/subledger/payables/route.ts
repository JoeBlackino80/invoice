...
// GET /api/subledger/payables - Dodávateľské saldokonto
export async function GET(request: Request) {
  ...
  // Fetch incoming invoices (prijaté faktúry)
  ...
  for (const inv of filteredInvoices) {
    ...
    contactMap[cId].total_payable += remaining
    if (isOverdue) {
  ...
  const data = Object.values(contactMap).sort((a, b) => b.total_payable - a.total_payable)

  // Calculate totals
  let grand_total_payable = 0
  let grand_total_overdue = 0
  let grand_invoice_count = 0

  for (const group of data) {
    grand_total_payable += group.total_payable
  ...
  return NextResponse.json({
    data,
    summary: {
      total_payable: grand_total_payable,
