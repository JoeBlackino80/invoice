...
import { ArrowLeft, Save, Plus, Trash2, ArrowRight, AlertTriangle } from "lucide-react"
...
export default function NewTransferPage() {
  ...
  useEffect(() => {
    const now = new Date()
    const num = `PV-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, "0")}${String(now.getDate()).padStart(2, "0")}-${String(Math.floor(Math.random() * 1000)).padStart(3, "0")}`
  ...
  const selectProduct = (index: number, product: ProductOption) => {
    const available = getProductStock(product, fromWarehouseId)
  ...
  // Update available stock when source warehouse changes
  useEffect(() => {
    ...
    setItems((prev) => prev.map((item) => {
      ...
      return { ...item, available_stock: getProductStock(product, fromWarehouseId) }
    }))
  }, [fromWarehouseId, products])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fromWarehouseId || !toWarehouseId) {
      toast({ variant: "destructive", title: "Chyba", description: "Vyberte zdrojovy a cielovy sklad" })
      return
    }

    if (fromWarehouseId === toWarehouseId) {
      toast({ variant: "destructive", title: "Chyba", description: "Zdrojovy a cielovy sklad nemozu byt rovnake" })
      return
    }
    ...
    if (insufficientItems.length > 0) {
      toast({
        ...
        description: "Niektore polozky nemaju dostatocne zasoby v zdrojovom sklade",
      })
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: "Prevodka vytvorena" })
        router.push("/warehouse")
      } else {
        const err = await res.json()
        toast({ variant: "destructive", title: "Chyba", description: err.error || "Nepodarilo sa vytvorit prevodku" })
      }
    } catch {
      toast({ variant: "destructive", title: "Chyba", description: "Nepodarilo sa vytvorit prevodku" })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Novy prevod</h1>
          <p className="text-muted-foreground">Prevod tovaru medzi skladmi</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-5xl">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Hlavicka prevodky</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center gap-4">
              <div className="flex-1 space-y-2">
                <Label>Zo skladu *</Label>
                <select
                  ...
                  required
                >
                  <option value="">Vyberte zdrojovy sklad</option>
                  {warehouses.map((wh) => (
              ...
              <div className="flex-1 space-y-2">
                ...
                <select
                  ...
                  required
                >
                  <option value="">Vyberte cielovy sklad</option>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="h-10 px-4 text-right font-medium">V zdrojovom sklade</th>
                                    ...
                                    filteredProducts.map((p) => (
                                      ...
                                      >
                                        ...
                                        <span className="text-xs text-muted-foreground ml-auto">
                                          {fromWarehouseId ? getProductStock(p, fromWarehouseId) : "?"} {p.unit}
                                        </span>
                                      </button>
                                    ))
                                  )}
        ...
        <div className="flex items-center gap-4">
          <Button type="submit" disabled={saving}>
            ...
            {saving ? "Ukladam..." : "Ulozit prevodku"}
          </Button>
          <Link href="/warehouse">
