...
export default function NewPredkontaciaPage() {
  ...
  const [lines, setLines] = useState<PredkontaciaLine[]>([createEmptyLine(), createEmptyLine()])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/predkontacie", {
        method: "POST",
      ...
      if (res.ok) {
        toast({ title: "Predkontacia vytvorena" })
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Nova predkontacia</h1>
          <p className="text-muted-foreground">Vytvorte novu sablonu pre automaticke uctovanie</p>
        </div>
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        <Button onClick={handleSave} disabled={saving}>
          ...
          {saving ? "Ukladam..." : "Ulozit predkontaciu"}
        </Button>
      </div>
    </div>
  )
