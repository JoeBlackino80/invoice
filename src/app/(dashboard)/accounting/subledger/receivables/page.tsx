...
export default function ReceivablesPage() {
  ...
  const fetchReceivables = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/subledger/receivables?${params}`)
      ...
      if (res.ok) {
        ...
        toast({ variant: "destructive", title: "Chyba", description: json.error || "Nepodarilo sa načítať pohľadávky" })
      }
    } catch {
      toast({ variant: "destructive", title: "Chyba", description: "Nepodarilo sa načítať pohľadávky" })
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Pohľadávky</h1>
          <p className="text-muted-foreground">Odberateľské saldokonto - prehľad neuhradených faktúr</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Celkové pohľadávky</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-2xl font-bold font-mono">{formatMoney(summary.total_receivable)}</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
                  ...
                  <tr>
                    <td colSpan={8} className="h-24 text-center text-muted-foreground">
                      <div>
                        ...
                        <p>Žiadne neuhradené pohľadávky.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                      <tbody key={group.contact_id}>
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right font-mono font-medium">{formatMoney(group.total_receivable)}</td>
